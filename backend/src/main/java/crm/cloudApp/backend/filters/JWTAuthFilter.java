package crm.cloudApp.backend.filters;

import crm.cloudApp.backend.services.auth.JWTService;
import crm.cloudApp.backend.dto.auth.JWTClaimsResponseDTO;
    ...
    private Authentication getAuthentication(HttpServletRequest request) {
        ...
        if (jwtClaimsResponseDTO != null && StringUtils.isNotBlank(jwtClaimsResponseDTO.getSubject())) {
            ...
            Optional<User> optionalUser =
                    userRepository.findByUsername(jwtClaimsResponseDTO.getSubject());
            ...
            List<GrantedAuthority> grantedAuths =
                    AuthorityUtils.commaSeparatedStringToAuthorityList(optionalUser.get().getModulesCrm());

            grantedAuths.addAll(
                    AuthorityUtils.commaSeparatedStringToAuthorityList(optionalUser.get().getModulesMarket())
            );

            grantedAuths.addAll(
                    AuthorityUtils.commaSeparatedStringToAuthorityList(optionalUser.get().getRolesCrm())
            );

            grantedAuths.addAll(
                    AuthorityUtils.commaSeparatedStringToAuthorityList(optionalUser.get().getRolesMarket())
            );
